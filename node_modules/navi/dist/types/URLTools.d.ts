/**
 * An object that store the parts of a URL that can correspond to a
 * a specific screen within your application.
 */
export declare type URLDescriptor = {
    /**
     * The URL's pathname part, starting from the first `/`, and ending
     * before any search string.
     *
     * E.g. `/documents/hidden/`
     */
    pathname: string;
    /**
     * The URL's query part, including any `?` character. When there
     * is no query, it defaults to an empty string.
     *
     * E.g. `?q=pants`.
     */
    search: string;
    /**
     * The URL's hash part, including any `#` character. When there
     * is no hash, it defaults to an empty string.
     *
     * E.g. `#top`.
     */
    hash: string;
    /**
     * An object containing string values of parameters extracted from
     * the `pathname` or `search` properties.
     */
    query: Params;
    hostname: string;
    /**
     * The full string URL. If this URL Descriptor was created from a
     * string that you passed in, then this will match your provided
     * string.
     */
    href: string;
};
export declare type TrailingSlashAction = 'add' | 'remove' | null;
export declare type Params = {
    [name: string]: string;
};
export interface URLDescriptorOptions {
    removeHash?: boolean;
    trailingSlash?: TrailingSlashAction;
}
export declare function createURLDescriptor(urlOrDescriptor: string | Partial<URLDescriptor>, { removeHash, trailingSlash }?: URLDescriptorOptions): URLDescriptor;
export declare function parseQuery(queryString?: string, leadingCharacter?: string): Params;
export declare function stringifyQuery(query: {
    [name: string]: any;
}, leadingCharacter?: string): string;
export declare function joinPaths(base: string, ...paths: string[]): string;
export declare function modifyTrailingSlash(pathname: string, action: 'add' | 'remove' | null): string;
